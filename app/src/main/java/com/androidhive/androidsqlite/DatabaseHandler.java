...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public DatabaseHandler(Context context, int databaseVersion) {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 2) {
            db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN " + KEY_ADD + " string;");
        }
        if (oldVersion > 1 && oldVersion < 3) {
            db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN " + KEY_GENDER + " string;");
        }
    }
